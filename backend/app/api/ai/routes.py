...
def scan_submission(submission_id):
    ...
    Admin / teacher only. Currently uses mock scores for testing.
    """
    current = get_current_user()
    if not current or current.role not in ("admin", "teacher"):
        return error("Forbidden", 403)
    ...
    try:
        fake_ai_score = round(random.uniform(0, 100), 2)
        fake_similarity_score = round(random.uniform(0, 100), 2)

        # 3. Save to database
        scan_result = SubmissionScan(
            submission_id=submission_id,
            ai_score=fake_ai_score,
            similarity_score=fake_similarity_score,
            status="completed"
        )
        ...
        return success({
            "submission_id": submission_id,
            "ai_score": float(fake_ai_score),
            "similarity_score": float(fake_similarity_score),
            "status": "completed"
        })

    except Exception as exc:
        db.session.rollback()
        return error(f"Scanning process failed: {exc}", 500)